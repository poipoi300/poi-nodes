...
def tensor2pil(tensor):
    # Check if the tensor has 3 or 4 dimensions
    if tensor.ndim == 4:  # Batch of images (B, H, W, C)
        tensor = tensor.permute(0, 3, 1, 2)  # Rearrange to (B, C, H, W)
...
def upscale_with_model(upscale_model, image):
    ...
    while oom:
        try:
            steps = in_img.shape[0] * comfy.utils.get_tiled_scale_steps(in_img.shape[2], in_img.shape[3], tile_x=tile, tile_y=tile, overlap=overlap)
...
def process_single_image(image, opt_mask, base_size, max_size, opt_upscale_model, scale_method):
    ...
    if base_size <= orig_width <= max_size and base_size <= orig_height <= max_size:
    ...
    if (orig_width < new_width or orig_height < new_height) and opt_upscale_model:
        resized_img = upscaler(image, opt_upscale_model,
...
class SmartResize:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "base_size": ("INT", {"default": 512, "min": 64, "max": 16384, "step": 64}),
                "max_size": ("INT", {"default": 1536, "min": 128, "max": 16384, "step": 64}),
                "scale_method": ([*resample_filters.keys()],{"default": "lanczos"}),
            },
            "optional": {
                "opt_upscale_model": ("UPSCALE_MODEL", {"default": None}),  # Optional upscaling model
    ...
    def resize_to_bounds(self, image, base_size, max_size, scale_method, opt_upscale_model=None, opt_mask=None):
        """
        Resizes an image (and optional mask) within given bounds.
        
        Args:
            img (tensor): The input image.
            base_size (int): Minimum allowed size for any dimension.
            max_size (int): Maximum allowed size for any dimension.
            upscale_model (Optional): An optional upscaling model (not used here).
            mask (Optional[PIL.Image.Image]): An optional mask to resize with the image.
        ...
        return torch.stack(processed_images), torch.stack(processed_masks) if processed_masks else None
